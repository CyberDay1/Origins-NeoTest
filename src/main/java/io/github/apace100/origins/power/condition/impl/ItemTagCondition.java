...
public final class ItemTagCondition implements Condition<ItemStack> {
    ...
    private ItemTagCondition(TagKey<Item> tag) {
        this.tag = tag;
    }
    ...
    public static ItemTagCondition fromJson(ResourceLocation id, JsonObject json) {
        if (!json.has("tag")) {
            Origins.LOGGER.warn("Item tag condition '{}' is missing required 'tag' field", id);
            return null;
        }
        ...
        try {
            ...
            Origins.LOGGER.warn("Item tag condition '{}' has invalid tag id '{}': {}", id, rawTag, exception.getMessage());
            return null;
        }

        return new ItemTagCondition(TagKey.create(Registries.ITEM, tagId));
    }

    public static Codec<ItemTagCondition> codec() {
        return CODEC;
    }
}
